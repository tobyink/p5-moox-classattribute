...
BEGIN {
	...
	$MooX::CaptainHook::VERSION   = '0.005';
}
...
use constant ON_APPLICATION => do {
	...
	BEGIN {
		...
		$MooX::CaptainHook::OnApplication::VERSION   = '0.005';
	}
	use Moo::Role;
		...
		role_metaroles => {
			role => eval q{
				...
				BEGIN {
					...
					$MooX::CaptainHook::OnApplication::Moose::VERSION   = '0.005';
				}
				use Moose::Role;
				after apply => sub {
					my $role    = $_[0]->name;
...
use constant ON_INFLATION => do {
	...
	BEGIN {
		...
		$MooX::CaptainHook::OnInflation::VERSION   = '0.005';
	}
	use Moo::Role;
